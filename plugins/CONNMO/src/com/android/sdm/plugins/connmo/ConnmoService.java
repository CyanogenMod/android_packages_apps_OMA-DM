...
public class ConnmoService extends Service {
    private final String TAG = "DM_ConnmoService";

    private IDmtPlugin.Stub mBinder;

    @Override
    public IBinder onBind(Intent intent) {
        if (ConnmoPlugin.class.getName().equals(intent.getAction())) {
            String rootPath = intent.getStringExtra("rootPath");
            Log.d(TAG, "rootPath: " + rootPath);
            if (mBinder == null) {
                Log.d(TAG, "mBinder is null, create it!");
                mBinder = new ConnmoPlugin(this);
            } else {
